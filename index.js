...
async function exportRealWebsite(repoName = "sweet-moment-chocolates", username = "your-username", customDomain = false) {
  console.log("\u{1F680} COMPLETE WEBSITE EXPORT - BUILDING REAL PRODUCTION VERSION");
  ...
  try {
    ...
    console.log("\u{1F528} Building production version of your website...");
    try {
      ...
      console.log("\u26A0\uFE0F Production build failed, creating manual export...");
      return await createManualExport(repoName, username, customDomain);
    }
    const distPath = path.join(process.cwd(), "dist");
    if (fs.existsSync(distPath)) {
      console.log("\u{1F4C1} Copying production build files...");
      fs.cpSync(distPath, outputDir, { recursive: true });
      console.log("\u2705 Production files copied");
    } else {
      console.log("\u274C No dist folder found, creating manual export...");
      return await createManualExport(repoName, username, customDomain);
    }
    ...
    await processFilesForStatic(outputDir, repoName, customDomain);
    await createZipFile(outputDir, zipPath);
    console.log("\u{1F389} REAL WEBSITE EXPORT COMPLETE!");
  } catch (error) {
    console.error("\u274C Export failed:", error);
    return await createManualExport(repoName, username, customDomain);
  }
}
async function createManualExport(repoName, username, customDomain) {
  console.log("\u{1F6E0}\uFE0F Creating manual static export...");
  const outputDir = path.join(process.cwd(), "static-site");
  ...
  try {
    console.log("\u{1F4CA} Fetching your website data...");
    ...
    console.log(`\u2705 Got ${products2.length} products and site configuration`);
    await createStaticHtmlFiles(outputDir, products2, siteData, basePath);
    await createRealCSS(outputDir, siteData);
    const uploadsSource = path.join(process.cwd(), "uploads");
    const uploadsTarget = path.join(outputDir, "uploads");
    if (fs.existsSync(uploadsSource)) {
      fs.cpSync(uploadsSource, uploadsTarget, { recursive: true });
    ...
    const zipPath = path.join(process.cwd(), "static-site.zip");
    await createZipFile(outputDir, zipPath);
    console.log("\u{1F389} MANUAL EXPORT COMPLETE!");
  } catch (error) {
    console.error("\u274C Manual export failed:", error);
    throw error;
  }
}
async function createStaticHtmlFiles(outputDir, products2, siteData, basePath) {
  console.log("\u{1F4DD} Creating static HTML files...");
  const heroSection = JSON.parse(siteData.heroSection || "{}");
  const featuredSection = JSON.parse(siteData.featuredSection || "{}");
  const indexHtml = createIndexPage(heroSection, featuredSection, products2, basePath);
  fs.writeFileSync(path.join(outputDir, "index.html"), indexHtml);
  const menuHtml = createMenuPage(products2, basePath);
  fs.writeFileSync(path.join(outputDir, "menu.html"), menuHtml);
  const aboutHtml = createAboutPage(basePath);
  fs.writeFileSync(path.join(outputDir, "about.html"), aboutHtml);
  const customOrderHtml = createCustomOrderPage(basePath);
  fs.writeFileSync(path.join(outputDir, "custom-order.html"), customOrderHtml);
  for (const product of products2) {
    const productHtml = createProductPage(product, basePath);
    fs.writeFileSync(path.join(outputDir, `product-${product.id}.html`), productHtml);
  }
  console.log("\u2705 All HTML files created");
}
async function createRealCSS(outputDir, siteData) {
  console.log("\u{1F3A8} Creating CSS that matches your website...");
  const css = `
/* Sweet Moment Chocolates - Real Website Styles */
:root {
...
.underline {
  width: 120px;
  height: 4px;
  background: linear-gradient(90deg, var(--accent-orange), var(--light-brown));
  margin: 0 auto;
  border-radius: 2px;
}
...
.product-card:hover {
  ...
  box-shadow: 0 24px 64px rgba(139, 69, 19, 0.2);
}
...
function createIndexPage(heroSection, featuredSection, products2, basePath) {
  const heroTitle = heroSection.title || "Sweet Moment Chocolates";
  const heroSubtitle = heroSection.subtitle || "Handcrafted Gourmet Chocolates Made with Love";
  const featuredTitle = featuredSection.title || "Featured Products";
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <header class="header">
        <div class="nav-container">
            <a href="${basePath}/index.html" class="logo">Sweet Moment</a>
            <nav class="nav-links">
                <a href="${basePath}/index.html">Home</a>
                <a href="${basePath}/menu.html">Menu</a>
                <a href="${basePath}/about.html">About</a>
                <a href="${basePath}/custom-order.html">Custom Order</a>
            </nav>
        </div>
    </header>
    ...
    <section class="hero">
        <div class="hero-content">
            <h1>${heroTitle}</h1>
            <p>${heroSubtitle}</p>
            <a href="${basePath}/menu.html" class="btn-primary">Explore Our Menu</a>
        </div>
    </section>
    ...
    <section class="featured-products">
        <div class="container">
            ...
            <div class="products-grid">
                ${products2.filter((p) => p.visible).slice(0, 6).map((product) => `
                    <div class="product-card">
                        <img src="${basePath}${product.imageUrl || "/uploads/placeholder.jpg"}" alt="${product.name}" class="product-image" />
                        <div class="product-content">
                            <h3 class="product-title">${product.name}</h3>
                            <p class="product-description">${product.description || "Premium handcrafted chocolate made with the finest ingredients"}</p>
                            <div class="product-price">From $${(product.basePrice / 100).toFixed(2)}</div>
                            <a href="${basePath}/product-${product.id}.html" class="btn-secondary">View Details</a>
                        </div>
                    </div>
                `).join("")}
            </div>
    ...
    <script>
        // Replace cart functionality with alerts for static site
        document.addEventListener('click', function(e) {
            if (e.target.textContent && e.target.textContent.includes('Add to Cart')) {
                e.preventDefault();
                alert('Please visit our main website to place orders!');
            }
        });
    </script>
</body>
...
function createMenuPage(products2, basePath) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Menu - Sweet Moment Chocolates</title>
    <link rel="stylesheet" href="${basePath}/style.css">
</head>
<body>
    <header class="header">
    ...
    <section class="page-header">
        <div class="container">
            <h1>Our Menu</h1>
            <p>Discover our handcrafted chocolate collection</p>
        </div>
    </section>
    ...
    <section class="page-content">
        <div class="container">
            <div class="products-grid">
                ${products2.filter((p) => p.visible).map((product) => `
                    <div class="product-card">
                        <img src="${basePath}${product.imageUrl || "/uploads/placeholder.jpg"}" alt="${product.name}" class="product-image" />
                        <div class="product-content">
                            <h3 class="product-title">${product.name}</h3>
                            <p class="product-description">${product.description || "Premium handcrafted chocolate"}</p>
                            <div class="product-price">From $${(product.basePrice / 100).toFixed(2)}</div>
                            <a href="${basePath}/product-${product.id}.html" class="btn-secondary">View Details</a>
                        </div>
                    </div>
...
function createAboutPage(basePath) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>About - Sweet Moment Chocolates</title>
    <link rel="stylesheet" href="${basePath}/style.css">
</head>
<body>
    <header class="header">
    ...
    <section class="page-header">
        <div class="container">
            ...
            <p>Creating moments of joy through handcrafted chocolates</p>
        </div>
    </section>
    
    <section class="page-content">
        <div class="container">
            <p>At Sweet Moment Chocolates, we believe every bite should be a celebration. Our master chocolatiers craft each piece with passion, using only the finest ingredients sourced from around the world.</p>
        </div>
    </section>
</body>
</html>`;
}
function createCustomOrderPage(basePath) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Custom Order - Sweet Moment Chocolates</title>
    <link rel="stylesheet" href="${basePath}/style.css">
</head>
<body>
    <header class="header">
    ...
    <section class="page-header">
        <div class="container">
            <h1>Custom Orders</h1>
            <p>Create something special for your occasion</p>
        </div>
    </section>
    
    <section class="page-content">
        <div class="container">
            <p>Please visit our main website to place custom orders and get in touch with our team.</p>
        </div>
    </section>
</body>
</html>`;
}
function createProductPage(product, basePath) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${product.name} - Sweet Moment Chocolates</title>
    <link rel="stylesheet" href="${basePath}/style.css">
</head>
<body>
    <header class="header">
    ...
    <section class="page-header">
        <div class="container">
            <h1>${product.name}</h1>
            <p>Premium handcrafted chocolate</p>
        </div>
    </section>
    ...
    <section class="page-content">
        <div class="container">
            <div class="product-details">
                <img src="${basePath}${product.imageUrl || "/uploads/placeholder.jpg"}" alt="${product.name}" style="max-width: 400px; border-radius: 16px;" />
                <div class="product-info">
                    <h2>${product.name}</h2>
                    <p>${product.description || "Premium handcrafted chocolate made with the finest ingredients"}</p>
                    <div class="product-price">From $${(product.basePrice / 100).toFixed(2)}</div>
                    <p><em>Please visit our main website to place orders.</em></p>
                </div>
            </div>
        </div>
    </section>
...
async function processFilesForStatic(outputDir, repoName, customDomain) {
  console.log("\u{1F527} Processing files for static hosting...");
  const basePath = customDomain ? "" : `/${repoName}`;
  ...
  for (const htmlFile of htmlFiles) {
    ...
    if (!customDomain && basePath) {
      content = content.replace(/href="\//g, `href="${basePath}/`).replace(/src="\//g, `src="${basePath}/`);
    }
    fs.writeFileSync(filePath, content);
  }
  console.log("\u2705 Files processed for static hosting");
}
async function createZipFile(sourceDir, zipPath) {
  console.log("\u{1F4E6} Creating ZIP file...");
...
async function registerRoutes(app2) {
  ...
  app2.use("/uploads", express5.static(path4.join(process.cwd(), "public/uploads")));
  app2.use("/static", express5.static(path4.join(process.cwd(), "public/static")));
    ...
    try {
      ...
      if (key === "themeSettings") {
        try {
          ...
          const themeJsonPath = path7.join(process.cwd(), "theme.json");
          fs6.writeFileSync(themeJsonPath, JSON.stringify(themeJson, null, 2));
  ...
  const setupRedirectPaths = async () => {
    try {
      ...
      commonPaths.forEach((path7) => {
        app2.get(path7, (req, res) => handleRedirect(req, res, null));
        app2.get(`${path7}/:name`, (req, res) => handleRedirect(req, res, req.params.name));
      });
